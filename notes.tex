...
The instrument response is encoded in the line spread function (LSF)
matrix $\mathbf{A} \in \mathbb{R}^{G \times G}$, defined on a fine
grid ($\Delta u = 0.05$, $u \in [-4, 64]$).  The \emph{effective
basis} $\mathbf{E}$ maps source coefficients directly to predicted
observations by convolving each basis function with the LSF:
\begin{equation}
  \label{eq:eff-basis-fine}
  E^\text{fine}_{jk} = \sum_m A_{jm}\, B_k(u^\text{fine}_m)\, \Delta u,
  \qquad j = 1, \ldots, G,
\end{equation}
then interpolating to the observation positions:
\begin{equation}
  \label{eq:eff-basis}
