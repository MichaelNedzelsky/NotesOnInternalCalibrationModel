...
For the reference CU, $\mathbf{A}^{\text{ref}}$ is the full LSF
convolution kernel $I_\text{ref} \cdot I_\text{ref}^\dagger$.  It is
\emph{not} the identity matrix: its entries are concentrated in a band
around the diagonal (diagonal values ${\sim}0.04$--$0.15$, row
sums~${\approx}1$).
...
shows that all three modes converge to
essentially the same final loss ($<\!0.5\%$ difference), demonstrating
...
\begin{enumerate}
  ...
  \item \textbf{Source Update:} For each source~$s$, stack all its
    ...
    base effective basis~$\mathbf{E}$.  Since all CUs use the same
    $\mathbf{E}$ (no correction yet), the source coefficients represent
    a compromise fit across all CUs.
...
\caption{Comparison of per-cu and identity Aij modes (1000 sources,
  ref CU = row~4 FoV~Following).  Per-cu achieves
