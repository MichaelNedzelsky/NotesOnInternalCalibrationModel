...
\num{1900759}~training and \num{814869}~test observations.  A
...
\begin{tabular}{lcc}
  ...
  Iterations            & 14 (converged) & 15 (converged)    \\
  Loss after iter 1     & $1.207 \times 10^6$ & $1.256 \times 10^6$ \\
  Final loss            & $1.100 \times 10^6$ & $1.113 \times 10^6$ \\
  Total loss reduction  & $-8.9\%$     & $-11.4\%$    \\
  \midrule
  Train data loss       & $7.555 \times 10^5$ & $7.653 \times 10^5$ \\
  Test data loss        & $3.348 \times 10^5$ & $3.374 \times 10^5$ \\
  Train loss/obs        & 0.3975       & 0.4027       \\
  Test loss/obs         & 0.4110       & 0.4140       \\
  Test/train ratio      & 1.034        & 1.028        \\
  \midrule
  Reg.\ penalty (final) & $9.650 \times 10^3$ & $1.018 \times 10^4$ \\
  \midrule
  BP sources increased  & 0/964        & 0/964        \\
  RP sources increased  & 0/932        & 0/932        \\
  \midrule
  Residual std          & 8.307        & 8.305        \\
  Residual MAD          & 2.000        & 2.000        \\
  $|r| > 5$             & 24.89\%      & 24.88\%      \\
  \bottomrule
\end{tabular}
\caption{Comparison of per-cu and identity Aij modes (1000 sources,
  ref CU = row~4 FoV~Following).  Per-cu converged at
  iteration~14 (relative loss change $< 10^{-4}$); identity converged
  at iteration~15.  Per-cu achieves
  ${\sim}1.2\%$ lower total loss by solving all 28~CUs with a nominal
...
\begin{tabular}{rrr}
  ...
  \midrule
  1  & $1.207 \times 10^6$ & $1.256 \times 10^6$ \\
  2  & $1.132 \times 10^6$ & $1.151 \times 10^6$ \\
  5  & $1.105 \times 10^6$ & $1.120 \times 10^6$ \\
  10 & $1.101 \times 10^6$ & $1.114 \times 10^6$ \\
  \bottomrule
\end{tabular}
...
test/train ratio stabilises at ${\sim}1.03$ in both modes, indicating
near-perfect generalisation of the shared instrument model.  Despite
the larger test fraction (30\% vs 20\%) and greater source diversity,
the 1000-source runs achieve \emph{lower} test/train ratios than the
200-source pilot (${\sim}1.08$), because more training sources
provide tighter constraints on $\Delta\mathbf{M}$.  The identity mode
has a marginally lower test/train ratio ($1.028$ vs $1.034$), because
...
\begin{tabular}{lcccc}
  ...
  Test loss/obs         & 0.390   & 0.394   & 0.411   & 0.414   \\
  Test/train ratio      & 1.087   & 1.073   & 1.034   & 1.028   \\
...
\begin{enumerate}
  ...
  \item \textbf{Per-observation loss increases.}  The train loss/obs
    ...
    population.
  \item \textbf{Generalisation improves.}  The test/train ratio
    \emph{decreases} from ${\sim}1.08$ at 200~sources to ${\sim}1.03$
    at 1000~sources, despite the larger test fraction (30\% vs 20\%).
    More training data constrains $\Delta\mathbf{M}$ better, yielding
    an instrument model that generalises more uniformly to unseen
    sources.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Per-cu mode achieves the lowest total loss}
    ($1.100 \times 10^6$) by solving all 28~CUs with a physically
    motivated nominal model.  The ${\sim}1.2\%$ advantage over identity
  ...
  \item \textbf{Residual statistics are identical.}  Both
    modes produce std~$\approx 8.31$, MAD~$= 2.000$, and
    $|r| > 5 \approx 24.89\%$.  Zero sources show increased chi2 in
    either mode.  The loss gap is spread uniformly across all sources
    and samples rather than concentrated in specific outliers.
  ...
  \item \textbf{No overfitting at either scale.}  Both training and
    test losses decrease monotonically in all runs.  The test/train
    ratio at 1000~sources (${\sim}1.03$) is \emph{lower} than at
    200~sources (${\sim}1.08$), indicating that the larger training
    set better constrains $\Delta\mathbf{M}$ and improves
    generalisation.
